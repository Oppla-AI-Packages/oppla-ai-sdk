import type { FlowStep, FlowTooltipStep, FlowModalStep, FlowBannerStep, FlowFeedbackStep, FlowAnnouncementListStep } from "../types";
...
export const isAnnouncementListStep = (step: FlowStep): step is FlowAnnouncementListStep =>
  "type" in step && step.type === "announcement-list";
export const isModalStep = (step: FlowStep): step is FlowModalStep =>
  !isTooltipStep(step) && ("title" in step || "body" in step) && !isBannerStep(step) && !isFeedbackStep(step) && !isAnnouncementListStep(step);
